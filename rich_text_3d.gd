...
## The fraction of characters to display, relative to the total number of characters.
@export_range(0, 1) var visible_ratio: float = 1.0:
	set(r): visible_ratio = r; _queue_render()
...
@export var font: Font:
	set(f):
		font = f
		if _text_2d: _text_2d.add_theme_font_override("normal_font", font)
		_queue_render()
...
@export_range(0, 20, 1) var outline_size := 0:
	set(o):
		outline_size = o
		if _text_2d: _text_2d.add_theme_constant_override("outline_size", outline_size)
		_queue_render()
